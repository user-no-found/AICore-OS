...
业务只读 capability 可以声明为 local process capability。`config.validate` 使用独立 `aicore-config-validate` component manifest，operation 为 `config.validate`，entrypoint 指向已安装的 `aicore-cli` application binary，args 指向内部 stdio handler。`auth.list`、`model.show`、`service.list`、`runtime.smoke`、`instance.list`、`cli.status`、`provider.smoke`、`agent.smoke`、`agent.session_smoke` 与 memory read operations 使用同类独立 component manifests，并通过内部 stdio handler 输出结构化只读或 smoke 结果。

Memory write capability 也可以声明为 local process capability，但必须遵守 write audit contract。`memory.remember`、`memory.accept` 与 `memory.reject` 的 manifest 必须使用独立 operation、`local_process` invocation mode、`stdio_jsonl` transport、已安装 application binary entrypoint 和内部 stdio handler args。此类 manifest 不改变 direct `aicore-cli config validate`、`aicore-cli auth list`、`aicore-cli model show`、`aicore-cli service list`、`aicore-cli runtime smoke`、`aicore-cli instance list`、`aicore-cli status`、`aicore-cli provider smoke`、`aicore-cli agent smoke`、`aicore-cli agent session-smoke`、direct memory read 或 direct memory write 本地命令语义。
...
stdin request 可以包含 operation-specific typed payload。typed payload 只用于 component handler 执行业务语义，不得被 invocation ledger 记录为 raw payload。包含用户内容的 write payload 必须限制在 component process stdin 边界和业务存储边界内，public result 应返回 content presence、length 或 safe id，而不是 raw content。
...
Memory write operation 的 public fields 必须表达 write audit contract。`memory.remember` 至少表达 operation、write_applied、audit_closed、write_outcome、idempotency、memory_id、memory_type、source、permanence、content_present、content_length 和 kernel_invocation_path。`memory.accept` 至少表达 operation、write_applied、audit_closed、write_outcome、idempotency、proposal_id、memory_id、status 和 kernel_invocation_path。`memory.reject` 至少表达 operation、write_applied、audit_closed、write_outcome、idempotency、proposal_id、status 和 kernel_invocation_path。write result 不应返回 raw memory content 或 proposal content；如果底层 memory event id 可用，可以作为 safe id 返回。

Kernel invocation ledger 只记录 invocation lifecycle，不替代 MemoryKernel 自己的 DB 和 memory event ledger。memory write 成功时，MemoryKernel 仍是业务事实源，invocation ledger 只表达 accepted、route decision、handler execution、event generation 和 invocation completion 等调用事实。write result 必须明确 `idempotency = not_guaranteed`，除非业务内核已经提供更强幂等保证；不得声称 exactly-once。
...
component handler 可以通过单行 JSONL result 返回 `status = "failed"` 来表达业务失败，例如 empty content、invalid proposal id 或 MemoryKernel write failure。此类失败应保留安全 public fields，例如 `write_applied = false`、`write_outcome = failed`、safe proposal id 和 idempotency 边界；Kernel invocation status 必须是 failed，不得伪装为 completed。
...
Memory write invocation ledger 可以记录 safe ids 和状态摘要，例如 operation、memory_id、proposal_id、event_id、write_applied、audit_closed 或 write_outcome；不得记录 raw memory content、proposal content、wiki markdown、raw stdout、raw stderr、raw runtime protocol payload、raw invocation request 或 secret material。
