...
顶层 `aicore` status 可以作为 kernel-native `runtime.status` consumer。该入口通过 installed manifest route、`KernelInvocationRuntime`、invocation ledger 与 result envelope 读取 runtime status；human status 文本从 structured result fields 派生。
