...
顶层 `aicore` status 通过 kernel-native `runtime.status` 消费 global runtime metadata。该路径会读取 installed manifests，经过 route decision runtime 与 invocation runtime，写入 invocation ledger，并从 result envelope 渲染用户可见状态。
