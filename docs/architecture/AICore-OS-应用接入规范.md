...
应用作为 component process 接入内核时，应通过 installed manifest 声明 invocation mode、transport、entrypoint、args 和 capability。`stdio_jsonl` component process 的 stdout 只承载协议 result，不承载 human panel。业务 read、smoke 或 write handler 应返回结构化 public fields，并由 CLI、TUI 或其他 surface 从 result envelope 派生人类摘要。

`config.validate`、`auth.list`、`model.show`、`service.list`、`runtime.smoke`、`instance.list`、`cli.status`、`provider.smoke`、`agent.smoke`、`agent.session_smoke`、memory read 类能力与 memory write 类能力可以由 application binary 暴露内部 stdio handler，并通过 Kernel runtime binary 调用。该内部 handler 不属于用户产品命令，不应出现在普通帮助 surface 中。handler 输出必须是单行 JSONL result，且不得输出 human panel、ANSI 或 raw credential material。
...
Memory write 类 handler 只表达现有 memory write CLI surface：`memory.remember`、`memory.accept` 与 `memory.reject`。它不代表 Memory Agent、proposal auto-accept、Vector、Graph、LLM Wiki、reverse sync、daemon 或 socket IPC 已接入。write handler 必须复用 MemoryKernel 的现有写入、accept 和 reject 语义；Kernel invocation ledger 只记录 invocation lifecycle，不替代 MemoryKernel DB 或 memory event ledger。write result 必须表达 write_applied、audit_closed、write_outcome、idempotency 和 safe id；不支持幂等时必须表达 `idempotency = not_guaranteed`。public result 和 ledger 不得记录 raw memory content、proposal content、raw payload、secret、`secret_ref`、credential lease、API key、token 或 cookie。
